...
class UserModel extends CI_Model {
    ...
    function inserData($data,$uitable) {
        ...
        $userinformation = array(
            ...
            'merital_status' => $uitable['merital_status']
        );
    ...
    function singleUsers($id) {
        $this->db->select('ui.personalDetails,ui.self,ui.partnerprefer,ui.aboutSelf,uf.aboutSisters,uf.aboutBrothers,ui.star,uf.alternateNo,ui.rashi,ui.area,ui.subcaste,ms.marital_status_name,ui.merital_status,uf.hobbies,ui.height,ui.gothram,m.mother_tongue_name,r.religion_name');
        $this->db->select('ui.birthtime,us.featureImage');
        $this->db->select('pt.age as page,pt.ageto as pageto,pt.religionId as preligionId,pt.merital_status as pmerital_status, pt.mothertoungeId as pmothertoungeId,pt.caste as pcaste,pt.location as plocation,pt.education as peducation,pt.otherDetails as potherDetails');
        ...
        $this->db->join('user_partner as pt', 'us.id = pt.userId', "left");
        ...
        function  allUsers($limit=null)
    {
        ...
        return $query->result();
            ...
            function  allFeatured($limit=null)
    {
        $this->db->select('ui.partnerprefer,ui.aboutSelf,uf.aboutSisters,uf.aboutBrothers,ui.star,uf.alternateNo,ui.rashi,ui.area,ui.subcaste,ms.marital_status_name,ui.merital_status,uf.hobbies,ui.height,ui.gothram,m.mother_tongue_name,r.religion_name');
        $this->db->select('ui.birthtime,us.featureImage');
        $this->db->select('ui.profission,ui.salary,uf.motherName,uf.fatherName,uf.motherOcc,uf.fatherOcc');
        ...
        function  partnerPrefer($id)
    {

        $this->db->select('*');
       $this->db->from('users as us');
        $this->db->join('user_partner as up', 'us.id = up.userId');       
        $this->db->join('castes as c', 'up.caste = c.id', "left");
        $this->db->join('religion as r', 'up.religionId = r.id', "left");
        $this->db->join('mother_tongue as m', 'up.mothertoungeId = m.id', "left");
        $this->db->join('marital_status as ms', 'up.merital_status = ms.id', "left");
        $this->db->where('us.id',$id);
        $this->db->limit(1);
        $query = $this->db->get();
        return $query->row();
